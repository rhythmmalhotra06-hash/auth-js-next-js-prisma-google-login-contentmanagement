...
export async function getTicketDetail(id: string): Promise<TicketDetail | null> {
  ...
  return {
    ...
    // The three per-ratio folder fields were deleted in Airtable (2026-08-28), so this
    // backend has nothing to read. The Postgres path still serves the app-local values.
    folder16x9: null,
    folder9x16: null,
    folder4x5: null,
    ...
    downloadLink: null, // "Download link" deleted in Airtable (2026-08-28); Postgres-only now
