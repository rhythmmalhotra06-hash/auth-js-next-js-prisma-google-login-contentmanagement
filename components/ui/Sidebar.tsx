  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[236px] flex-col border-r border-border-default bg-surface lg:flex">
      <div className="flex h-[60px] items-center gap-2.5 border-b border-border-default px-5 py-3">
        ...
        <span className="text-base font-bold tracking-tight text-text">Content Engine</span>
      </div>
      ...
      <div className="border-t border-border-default px-3 py-3">
        {email && <div className="truncate px-2 pb-2 text-2xs text-text-subtle" title={email}>{email}</div>}
        <form action={signOutAction}>
          <button
            type="submit"
            className="flex w-full items-center gap-3 rounded-sm px-3 py-2 text-sm text-text-muted transition-colors hover:bg-bg-subtle hover:text-text"
          >
            <span className="w-4 text-center text-sm opacity-80">⏻</span>
            Sign out
          </button>
        </form>
        <div className="px-2 pt-2 text-2xs text-text-subtle">Mindvalley · Creative Services</div>
      </div>
    </aside>
  );
}
