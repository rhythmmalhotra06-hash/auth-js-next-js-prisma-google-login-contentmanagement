...
export function FunnelCapacity({ tickets }: { tickets: QueueTicket[] }) {
  const stages = [
  ...
  for (const t of active) if (t.assignee) byEditor.set(t.assignee, (byEditor.get(t.assignee) ?? 0) + 1);
  ...
  return (
    <div className="datarow">
      ...
      <div className="panel">
        ...
        {caps.map(([name, load]) => {
          const pct = Math.min((load / 4) * 100, 100);
          const col = load >= 4 ? 'var(--red)' : load >= 3 ? 'var(--amber)' : 'var(--green)';
          return (
