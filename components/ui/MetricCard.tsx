...
export function MetricCard({ label, value, trend, trendLabel, className }: {
  ...
  return (
    <div className={cn('rounded-[12px] border border-border-default bg-surface p-4 shadow-[var(--mv-shadow-light)]', className)}>
      ...
      <div className="mt-1.5 flex items-end justify-between gap-2">
        ...
        {trend && trendLabel && (
          <span className={cn('rounded-[8px] px-2 py-0.5 text-xs font-medium', TREND[trend])}>{trendLabel}</span>
        )}
      </div>
    </div>
  );
