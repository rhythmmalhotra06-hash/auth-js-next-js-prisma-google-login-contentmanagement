...
import { updatePrioStatus } from '@/app/tickets/[id]/actions';
import { PRIO_STATUSES } from '@/lib/tickets/constants';

export function PrioStatusUpdater({ ticketId, current }: { ticketId: string; current: string | null }) {
  ...
  function onChange(next: string) {
    ...
    setMsg(null);
    start(async () => {
      const res = await updatePrioStatus(ticketId, next);
