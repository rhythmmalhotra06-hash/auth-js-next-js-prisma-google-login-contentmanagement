  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button
        onClick={run}
        disabled={pending}
        className="shrink-0 rounded-lg px-4 py-2.5 text-sm font-medium text-[#572280] ring-1 ring-[#572280]/30 hover:bg-[#572280]/5 disabled:opacity-50"
      >
        {pending ? 'Converting…' : 'Convert checked now'}
      </button>
      {msg && (
        <span className={`text-xs ${msg.kind === 'ok' ? 'text-green-600' : 'text-amber-600'}`}>
          {msg.text}
        </span>
      )}
    </div>
