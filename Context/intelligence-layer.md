...
The five capabilities compound in a loop:

```
        ┌─────────────────────────────────────────────┐
        ▼                                             │
  [1] Performance insight ──▶ [2] Brief generation ──▶ better assets
        ▲                                             │
        │                                             ▼
  [4] DNA feedback ◀── assets produced ── [3] Learning prioritization
        │
        └──▶ [5] Conversational layer (brain) sits across all of it
```

Build order: **1 and 2 first** (they compound and use data already connected),
then 3, then 4, with 5 emerging naturally once data lives in the brain.
